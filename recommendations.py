...
def recomend_by_genre(genre, flight):
    ...
    connect = pymysql.connect(host=config.host,
                              ...
                              db=config.db)
    cursor = connect.cursor()
    query = "SELECT * from films WHERE geners_id = '{}'" \
            " AND film_duration BETWEEN {} AND {} ORDER BY rating DESC".format(genre_id, duration_bottom, duration_flight)
    cursor.execute(query)
    ...
    if rows:
        ...
        for row in rows:
            print(row)
            response_item = {'film_name': row[2], 'rating': str(row[3]), 'duration': str(row[4]), 'link': row[6]}
